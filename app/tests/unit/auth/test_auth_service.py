...
def setup_async_mocks(mocker: MockerFixture):
    ...
    mocker.patch(
        f"{SERVICE_PATH}.run_in_threadpool", side_effect=fake_run_in_threadpool
    )
    ...
    mock_is_revoked = mocker.patch(
        f"{SERVICE_PATH}.is_token_revoked", return_value=False
    )
...
def test_login_success(
    ...
    mock_verify = mocker.patch(
        "app.domains.auth.service.verify_password", return_value=True
    )
    mock_uuid7 = mocker.patch(
        "app.domains.auth.service.uuid7", return_value="mocked-jti-uuid"
    )
    mock_create_access = mocker.patch(
        "app.domains.auth.service.create_access_token", return_value="access-jwt"
    )
    mock_create_refresh = mocker.patch(
        "app.domains.auth.service.create_refresh_token", return_value="refresh-jwt"
    )
    mock_set_cookie = mocker.patch("app.domains.auth.service.set_refresh_cookie")

    # Mock the user_agent_parser
    mocker.patch(
        "app.domains.auth.service.user_agent_parser.ParseOS",
        return_value={"family": "Windows"},
    )
    mocker.patch(
        "app.domains.auth.service.user_agent_parser.ParseDevice",
        return_value={"family": "PC"},
    )

    # 3. EXECUTE
    result = AuthService.login(
        request=mock_request, response=mock_response, db=mock_db, form_data=form_data
    )
...
async def test_refresh_success(
    ...
    # Assertions
    mock_revoke_tokens.assert_called_once_with(
        mock_redis, "jti-123", 9999999999
    )  # Did we kill the old token?
    assert (
        fake_auth_session.refresh_token_jti == "new-jti-456"
    )  # Did we update the DB session?
    mock_db.commit.assert_called_once()  # Did we save it?
    mock_set_cookie.assert_called_once_with(
        mock_response, "new-refresh-jwt"
    )  # Did we set the new cookie?
